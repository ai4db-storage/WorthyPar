...
def get_predicted_querys(sqls, tables):
    predicted_query = []
    query_list = [13, 48, 96]
    for i in query_list:
        sql_name = "sql" + str(i)
        sql = sqls[sql_name]
        print(sql[0])
        predicted_query.append(get_predicted_query(sql[0], sql[1], tables))
    """
    i = 1
...
def get_predicted_query(sql, radio, actual_tables):
    """
    :return: predicted_query（select，from，where）
    """
    ...
    where_columns = []
    if " order" in sql:
        query_sql = "select *" + sql[sql.find(" from"):sql.find(" order")] + ";"
    else:
        query_sql = "select *" + sql[sql.find(" from"):]
    ...
    print("Select columns：", len(select_columns))
    print("Select columns：", select_columns)
    print("where columns：", len(where_columns))
    print("where columns：", where_columns)
    print("columns alias：", alias_columns)
    print("tables alias：", alias_tables)
    print("cte tables：", cte_tables)
    # ==================================================================================================================
    # predicted_query:[select,from,where]
    #                 select [[1,2],[2,3]]  Represents: T1.A2, T2.A3
    #                 from   [1,2]          Represents: T1, T2
    #                 where  [1，[2，[3]]]   Represents: T1.A2 and the query result is in horizontal block 3
    query = []
    ...
    for column in select_columns:
        ...
        if table_name != 'constant':
            select.append([list(tables.keys()).index(table_name) + 1, tables[table_name][1][1].index(column_name) + 1])
    query.append(select)

    table = []
    for table_name in table_names:
        table.append(list(tables.keys()).index(table_name) + 1)
    query.append(table)
    ...
    for column in where_columns:
        ...
        column_name = column[1]
        if table_name != 'constant':
            where.append([list(tables.keys()).index(table_name) + 1, [tables[table_name][1][1].index(column_name) + 1]])
    ...
    query_matchs = dict()
    for query_column_name in query_columns:
        for table_name in table_names:
            if query_column_name in tables[table_name][1][1]:
                if table_name in query_matchs.keys():
                    query_matchs[table_name].append(query_column_name)
                else:
                    query_matchs.update({table_name: [query_column_name]})
    query_results = dict()
    for match_table, match_columns in query_matchs.items():
        partitions = []
        for row in query_rows:
            match_values = tuple(row[query_columns.index(column)] for column in match_columns)
            placeholders = ', '.join(['%s'] * len(match_columns))
            query_loc = f"""
                    SELECT row_num
                    FROM (  
                    ...
                    WHERE ({", ".join(match_columns)}) = ({placeholders})
                """
            print(query_loc, match_values)
            cur.execute(query_loc, match_values)
            result_test = cur.fetchall()
            print(match_table, result_test)
            for row_num in result_test:
                partitions.append(math.ceil(row_num[0] * 10 / tables[match_table][0]))
        query_results.update({match_table: list(set(partitions))})
    cur.close()
    conn.close()

    for wh in where:
        wh_table = list(tables.keys())[wh[0] - 1]
        wh[1].append(query_results[wh_table])
        ...
        if " and " in where_item and "select" not in where_item and where_item.count(" and ") != where_item.count(
            ...
            if " in " in where_item:
                ...
                if "select" in object2 and "where" in object2 and "group" in object2:
                    ...
                    if "cast" in object2:
                        object2, _, _ = process_cast(object2)
                    if "case" in object2:
...
def is_expression(str):
    ...
    if index != -1:
        while not (is_match(str[:index].strip()) and is_match(str[index + 1:].strip())):
            ...
            if index == -1:
                return False
        if ("," in str[:index].strip() and "(" not in str[:index].strip()) or (
                "," in str[index + 1:].strip() and "(" not in str[index + 1:].strip()):
            return False
        return True
    return False
...
def find_which_table(column):
    ...
    if "." in column:
        ...
        if table in cte_tables.keys():
            ...
            print("ERROR: undefined column name", column)

    for table_name in tables.keys():
        if column in tables[table_name][1][1]:
...
def analysis_select(sql):
    ...
    if re.search(r'with (.*) select', sql, re.DOTALL | re.IGNORECASE):
        match = re.search(r'with (.+)', sql)
        _, sql = find_all_with(match.group(1))
    if select_node.this:
    ...
    for select_column in select_node.selects:
        ...
        if select_column.strip() == '*':
            ...
            if "CAST" in select_column or "cast" in select_column:
                select_column = process_cast(select_column)
            if "CASE" in select_column or "case" in select_column:
...
def create_cte_table(cte_name, condition):
    cte_columns = dict()
    columns_group = find_all_selects(condition)
    cte_column_names = []
    if columns_group[0] == "*":
        for table in sqlglot.parse_one(condition).find_all(exp.Table):
            if "AS" in table.sql():
                star_name, _ = get_table_aliasName(table.sql())
            else:
                star_name = table.sql()
            table_columns = find_table_columns(star_name)
            cte_column_names.extend(table_columns)
    else:
        for columns in columns_group:
            columns_list = find_all_columns(columns)
            for column in columns_list:
                column = column.strip()
                if "cast" in column:
                    column = process_cast(column)
                if "case" in column:
                    column, _ = process_case(column)
                if " " in column:
                    origin, alias = get_column_aliasName(column)
                    if alias in alias_columns.keys():
                        alias_columns[alias].extend(origin)
                    else:
                        alias_columns.update({alias: origin})
                    column = alias
                if columns_group.index(columns) == 0:
                    cte_column_names.append(column)
    for cte_column_name in cte_column_names:
        origin_columns = find_alias_column(cte_column_name)
        cte_column = []
        for origin_column in origin_columns:
            cte_column.extend(find_which_table(origin_column))
        cte_columns.update({cte_column_name: cte_column})
    cte_tables.update({cte_name: cte_columns})
